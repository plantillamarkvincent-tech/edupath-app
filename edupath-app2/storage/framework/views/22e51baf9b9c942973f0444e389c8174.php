...
<?php if($errors->any()): ?>
    <div class="max-w-4xl mx-auto mt-4 mb-4 p-4 bg-red-100 text-red-700 rounded-lg text-sm">
        <div class="font-semibold mb-1">Please fix the following before submitting:</div>
...
<?php if(session('status')): ?>
    <div class="max-w-4xl mx-auto mt-4 mb-4 p-4 bg-emerald-100 text-emerald-700 rounded-lg text-sm font-medium">
...
<style>
    ...
    /* Compact Document Code box (matches second image) */
    .doc-control {
    ...
    /* SPF header styles (matched to submitted SPF) */
    .form-header {
        display: flex;
        justify-content: space-between;
        align-items: center;
...
<div class="spf-form">
    <!-- Header -->
    <?php echo $__env->make('admin.enrollment_requests._spf_header', [
    ...
    <div style="position: relative; margin-bottom: 15px; min-height: 150px;">
        ...
        <div class="photo-box" id="photoBox" style="float: right; margin: 0; position: relative;">
            <img id="photoPreview" class="photo-preview" alt="Student Photo" style="display: none;" />
    ...
    <form method="POST" action="<?php echo e(route('enrollment.store')); ?>" id="spfForm" enctype="multipart/form-data">
		<?php echo csrf_field(); ?>
		<input type="hidden" name="program_id" value="<?php echo e($program->id); ?>" />
		
		<!-- Photo upload input - positioned over the photo box -->
		<input type="file" name="student_photo" id="studentPhoto" accept="image/*" 
		       style="position: fixed; top: 0; right: 0; opacity: 0; width: 1px; height: 1px;" 
		       onchange="document.getElementById('photoPreview').src = URL.createObjectURL(this.files[0]); document.getElementById('photoPreview').style.display = 'block'; document.getElementById('photoText').style.display = 'none';" />
        ...
        <div class="submit-buttons no-print">
            ...
            <a href="<?php echo e(route('career.index')); ?>">Cancel</a>
		</div>
	</form>
</div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Admin\edupath-app\edupath-app2\resources\views/enrollment/create.blade.php ENDPATH**/ ?>