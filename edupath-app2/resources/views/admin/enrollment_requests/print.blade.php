...
<style>
    ...
    @media print {
        ...
        /* Hide browser header/footer elements when printing */
        nav, header, footer, .navbar, .navigation {
            display: none !important;
        }
        /* Hide Laravel app layout wrapper elements */
        .max-w-7xl, .py-12, .bg-gray-50, .container {
            display: none !important;
        }
        /* Clean page margins for printing */
        html, body {
            margin: 0 !important;
            padding: 0 !important;
        }
...
<div class="spf-form">
    ...
    <div style="position: relative; margin-bottom: 15px; min-height: 150px;">
        ...
        <!-- Photo Box -->
        <div class="photo-box" id="photoBox" style="float: right; margin: 0;">
    ...
    @if(isset($enrollmentRequest))
        @include('admin.enrollment_requests._spf_display', ['enrollmentRequest' => $enrollmentRequest])
    @else
    <form method="POST" action="{{ route('enrollment.store') }}" id="spfForm" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="program_id" value="{{ isset($program) ? $program->id : ($enrollmentRequest->program_id ?? '') }}" />
        ...
        <!-- rest of form is unchanged... for brevity it's left as in the original -->
...
<script>
    ...
    // Photo upload preview (only used in edit form, safe to keep)
...
<body>
    <div class="print-header no-print">
        <button onclick="window.print()">🖨️ Print SPF</button>
    ...
    <div class="spf-form">
        ...
        <div class="section">
            <div class="section-title">I. APPLICATION FOR ADMISSION</div>
            <div class="form-field"><label>1. DOrSU Student ID Number:</label><span class="form-value">{{ $enrollmentRequest->student_id ?? '________________' }}</span></div>
            <div class="form-field"><label>2. Semester:</label><span class="form-value">{{ $enrollmentRequest->semester ?? '________________' }}</span> <label style="margin-left: 20px;">Academic Year:</label><span class="form-value">{{ $enrollmentRequest->academic_year ?? '________________' }}</span></div>
            <div class="form-field"><label>3.</label> 
                @if(is_array($enrollmentRequest->student_type))
                    @if(in_array('first_year', $enrollmentRequest->student_type)) ✓ First Year @endif
                    @if(in_array('transferee', $enrollmentRequest->student_type)) ✓ Transferee @endif
                    @if(in_array('returnee', $enrollmentRequest->student_type)) ✓ Returnee @endif
                @else
                    ________________
                @endif
            </div>
            <div class="form-field"><label>4. LRN:</label><span class="form-value">{{ $enrollmentRequest->lrn ?? '________________' }}</span></div>
            <div class="form-field"><label>5. Campus:</label><span class="form-value">{{ $enrollmentRequest->campus ?? '________________' }}</span></div>
            <div class="form-field">
                <label>6. Preferred Courses:</label><br>
                <div style="margin-left: 20px;">
                    <div>1. <span class="form-value">{{ $enrollmentRequest->preferred_course_1 ?? '________________' }}</span></div>
                    <div>2. <span class="form-value">{{ $enrollmentRequest->preferred_course_2 ?? '________________' }}</span></div>
                    <div>3. <span class="form-value">{{ $enrollmentRequest->preferred_course_3 ?? '________________' }}</span></div>
                </div>
            </div>
        </div>
        ...
        <div class="section">
            ...
            <div class="form-field">
                <label>SURNAME:</label><span class="form-value">{{ $enrollmentRequest->surname ?? '________________' }}</span>
                <label style="margin-left: 20px;">FIRST NAME:</label><span class="form-value">{{ $enrollmentRequest->first_name ?? '________________' }}</span>
                <label style="margin-left: 20px;">MIDDLE NAME:</label><span class="form-value">{{ $enrollmentRequest->middle_name ?? '________________' }}</span>
            </div>
            <div class="form-field"><label>8. Date of Birth:</label><span class="form-value">{{ $enrollmentRequest->date_of_birth ? $enrollmentRequest->date_of_birth->format('m/d/Y') : '________________' }}</span></div>
            <div class="form-field"><label>9. Sex:</label> {{ $enrollmentRequest->sex === 'male' ? '✓ Male' : ($enrollmentRequest->sex === 'female' ? '✓ Female' : '________________') }}</div>
            <div class="form-field"><label>10. Place of Birth:</label><span class="form-value">{{ $enrollmentRequest->place_of_birth ?? '________________' }}</span></div>
            <div class="form-field">
                <label>(Municipality/City):</label><span class="form-value">{{ $enrollmentRequest->birth_municipality ?? '________________' }}</span>
                <label style="margin-left: 20px;">(Province):</label><span class="form-value">{{ $enrollmentRequest->birth_province ?? '________________' }}</span>
                <label style="margin-left: 20px;">(Country):</label><span class="form-value">{{ $enrollmentRequest->birth_country ?? '________________' }}</span>
            </div>
            <div class="form-field"><label>11. Civil Status:</label> 
                {{ $enrollmentRequest->civil_status === 'single' ? '✓ Single' : '' }}
                {{ $enrollmentRequest->civil_status === 'married' ? '✓ Married' : '' }}
                {{ $enrollmentRequest->civil_status === 'widowed' ? '✓ Widowed' : '' }}
                {{ $enrollmentRequest->civil_status === 'separated' ? '✓ Separated/Annulled' : '' }}
                @if(!$enrollmentRequest->civil_status) ________________ @endif
            </div>
            <div class="form-field"><label>12. Citizenship:</label><span class="form-value">{{ $enrollmentRequest->citizenship ?? '________________' }}</span></div>
            <div class="form-field"><label>13. Height (ft):</label><span class="form-value">{{ $enrollmentRequest->height ?? '________________' }}</span> <label style="margin-left: 20px;">14. Weight (kg):</label><span class="form-value">{{ $enrollmentRequest->weight ?? '________________' }}</span></div>
            <div class="form-field"><label>15. Religion:</label><span class="form-value">{{ $enrollmentRequest->religion ?? '________________' }}</span></div>
            <div class="form-field"><label>16. Tribe/Ethnic Group:</label><span class="form-value">{{ $enrollmentRequest->tribe_ethnic ?? '________________' }}</span></div>
            <div class="form-field"><label>17. E-mail Address:</label><span class="form-value">{{ $enrollmentRequest->email ?? '________________' }}</span></div>
            <div class="form-field"><label>18. Contact Number:</label><span class="form-value">{{ $enrollmentRequest->contact_number ?? '________________' }}</span></div>
            <div class="form-field"><label>19. Permanent Address:</label><span class="form-value" style="min-width: 400px;">{{ $enrollmentRequest->permanent_address ?? '________________' }}</span> <label style="margin-left: 20px;">Zip Code:</label><span class="form-value">{{ $enrollmentRequest->zip_code ?? '________________' }}</span></div>
        </div>

        <!-- Additional sections would continue here with similar formatting -->
        <!-- For brevity, showing key sections. You can expand this to include all sections from the form -->

        <div style="text-align: center; font-style: italic; font-weight: bold; margin-top: 20px; font-size: 11px;">
        ...
        <div class="signature-section">
            <div class="signature-box">
                ...
                <div style="margin-top: 5px; font-size: 10px;">{{ $enrollmentRequest->full_name }}</div>
            </div>
            <div class="signature-box" style="text-align: right;">
                <div>Date Signed</div>
                <div class="signature-line"></div>
                <div style="margin-top: 5px; font-size: 10px;">{{ $enrollmentRequest->created_at->format('m/d/Y') }}</div>
            </div>
        </div>
    </div>
</body>
...
<div class="spf-form">
    ...
    <div style="position: relative; margin-bottom: 15px; min-height: 150px;">
        ...
        <!-- Photo Box -->
        <div class="photo-box" id="photoBox" style="float: right; margin: 0;">
            <input type="file" name="student_photo" id="studentPhoto" accept="image/*" />
            <img id="photoPreview" class="photo-preview" alt="Student Photo" />
            <div class="photo-upload-text" id="photoText" style="color: #000;">
                Paste your<br><strong>2x2 photo</strong><br>here
            </div>
        </div>
        <div style="clear: both;"></div>
    </div>
